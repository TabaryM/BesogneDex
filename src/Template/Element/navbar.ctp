<nav class="navbar navbar-light navbar-expand-md bg-light" style="margin-top: 10px;margin-right: 5px;margin-left: 5px;">
    <div class="container-fluid"><button data-toggle="collapse" class="navbar-toggler" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
        <div class="collapse navbar-collapse" id="navcol-1">
            <ul class="nav navbar-nav d-flex justify-content-around" style="width: 100%;">
                <li class="nav-item" role="presentation"><a class="nav-link active" href="#">Accueil</a></li>
                <li class="nav-item" role="presentation"><a class="nav-link" href="#">Mes projets</a></li>
                <li class="nav-item" role="presentation"><a class="nav-link" href="#">Mes tâches</a></li>
                <li class="nav-item" role="presentation"><a class="nav-link" href="#">A propos</a></li>
            </ul>
        </div>
    </div>
</nav>
